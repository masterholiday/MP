...
$(function() {
  ...
  $(document).bind('click', function(e) {
		...
		if (!clicked.parents().hasClass('dropdown')) {
            if ($('span.selectbox-' + _classname).length > 0 && $('span.selectbox-' + _classname + ' ul.dropdown').is(':visible')) {
	...
	$('select.' + _classname).each(function() {
		...
		$(this).before(
			'<span class="selectbox-' + _classname + '" style="display: inline-block; position: relative">'+
				'<span class="select" style="float: left; position: relative;"><span class="text">' + selectText.replace("*", "<i>*</i>") + '</span>'+
