function saveArticle() {
	...
	$.ajax({
		type: 'POST',
		url: HTTP_HOST + '/site-manager/Articles/save',
		data: params,
		success: function(data, textStatus){processSaveArticle(data);},
		dataType: 'json'
	});
}

function processSaveArticle(data) {
	if (data.error != 0) {
		alert(data.errorStr);
		return;
	}
...
$(function(){
	...
	$(".del_article").click(function(){
		...
		if(confirm('Вы действительно хотите удалить статью з заголовком '+name+'?')){
			showLoader();
			$.ajax({
				type: 'POST',
				url: HTTP_HOST + '/site-manager/Articles/delete',
				data: {id:id},
				success: function(data, textStatus){processSaveArticle(data);},
				dataType: 'json'
			});
	...
	$(".state").click(function(){
		...
		$.ajax({
			type: 'POST',
			url: HTTP_HOST + '/site-manager/Articles/state-changer',
			data: {id:id},
			success: function(data, textStatus){processSaveArticle(data);},
			dataType: 'json'
		});		
	...
	$("#group_delete").click(function(){
		...
		if (arrCheckbox.length != 0){
			if(confirm('Вы действительнно хотите удалить выбраные новости?')){
				...
				$.ajax({
					type: 'POST',
					url: HTTP_HOST + '/site-manager/Articles/grouped-delete',
